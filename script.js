...
function bindEvents() {
    ...
    if (backBtn) backBtn.addEventListener('click', (e) => { e.preventDefault(); showExitModal(); });
    else console.warn('backBtn غير موجود');

    const backToHome = document.getElementById('backToHome');
    if (backToHome) backToHome.addEventListener('click', (e) => { e.preventDefault(); showExitModal(); });

    const themeToggle = document.getElementById('themeToggle');
    if (themeToggle) themeToggle.addEventListener('click', toggleTheme);

    // ربط أزرار متعددة باستخدام محددات
    ...
    if (createFileBtn) createFileBtn.addEventListener('click', (e) => { e.preventDefault(); createNewFile(); });
    ...
    // أزرار الجوال الإضافية
    const mobileFormat = document.getElementById('mobileFormat');
    if (mobileFormat) mobileFormat.addEventListener('click', () => {
        formatCode();
    ...
    if (mobileFullscreen) mobileFullscreen.addEventListener('click', () => {
        toggleFullscreen();
        document.getElementById('mobileDropdown')?.classList.remove('show');
    });
...
function initMobile() {
    ...
    // ربط أزرار الجوال (تم ربطها مسبقاً، نضيف هنا للتأكيد)
