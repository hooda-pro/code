// ===== محرر الأكواد الذكي – صفحة المحرر v3.0 =====
...
function bindEvents() {
  ...
  document.getElementById('refreshFiles').addEventListener('click', () => updateUI());
...
function updateFilesList() {
  ...
  EditorState.files.forEach((file, index) => {
    ...
    item.querySelector('.close-file').addEventListener('click', (e) => {
      e.stopPropagation();
...
function updateCurrentFile() {
  ...
  if (!EditorState.currentFile) {
    codeInput.value = '';
    currentFileName.textContent = 'اختر ملفاً';
    languageSelect.value = 'html';
    return;
  }
  codeInput.value = EditorState.currentFile.content;
  currentFileName.textContent = EditorState.currentFile.name;
  languageSelect.value = EditorState.currentFile.type;
  document.getElementById('currentLanguage').textContent = getLanguageName(EditorState.currentFile.type);
}

function updateProjectDetails() {
  const details = document.getElementById('projectDetails');
  if (!details || !EditorState.project) return;
  const date = new Date(EditorState.project.updatedAt);
...
function filterFiles(query) {
  ...
  if (filtered.length > 0) {
    filtered.forEach((file, idx) => {
      ...
      item.querySelector('.open-manager-file').addEventListener('click', () => {
        hideAllModals();
        switchToFile(originalIndex);
      });
      item.querySelector('.delete-manager-file').addEventListener('click', () => {
...
function toggleFullscreen() {
  ...
  if (!document.fullscreenElement) {
    container.requestFullscreen().catch(err => showToast('لا يمكن ملء الشاشة', 'error'));
...
function initMobile() {
  ...
  document.getElementById('bottomPreview')?.addEventListener('click', openPreview);
  document.getElementById('bottomAddFile')?.addEventListener('click', () => showModal('newFileModal'));
}
