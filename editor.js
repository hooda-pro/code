...
function loadProject() {
    try {
        ...
        if (saved) {
            window.editorState.currentProject = JSON.parse(saved);
...
function initModals() {
    ...
    document.getElementById('newFileForm')?.addEventListener('submit', e => { e.preventDefault(); createNewFile(); });
    document.getElementById('searchForm')?.addEventListener('submit', e => { e.preventDefault(); performSearch(); });
}
...
function initKeyboardShortcuts() {
    document.addEventListener('keydown', function(e) {
        ...
        if ((e.ctrlKey || e.metaKey) && e.key === 'h') { e.preventDefault(); showModal('searchModal'); document.getElementById('replaceText').focus(); }
        if ((e.ctrlKey || e.metaKey) && e.key === 'n') { e.preventDefault(); showModal('newFileModal'); }
        if (e.key === 'F11') { e.preventDefault(); toggleFullscreen(); }
        if (e.key === 'Escape' && document.querySelector('.modal.active')) hideAllModals();
...
function initSidebar() {
    ...
    document.getElementById('fileSearch')?.addEventListener('input', function() { filterFiles(this.value); });
}

// ===== فتح المعاينة في تبويب جديد =====
function openPreviewInNewTab() {
    if (!window.editorState.currentProject) {
        showToast('لا يوجد مشروع للمعاينة', 'error');
        return;
    }
    saveProject();
    ...
    if (!previewWindow) showToast('يرجى السماح بفتح النوافذ المنبثقة', 'warning');
}
...
function updateFilesList() {
    ...
    window.editorState.files.forEach((file, index) => {
        ...
        item.addEventListener('click', function(e) { if (!e.target.closest('.file-actions')) switchToFile(parseInt(this.dataset.index)); });
        item.querySelector('.close-file').addEventListener('click', e => { e.stopPropagation(); closeFile(parseInt(item.dataset.index)); });
    });
}
...
function updateEditorTabs() {
    ...
    window.editorState.files.forEach((file, index) => {
        if (file.active) {
            ...
            tab.addEventListener('click', function(e) { if (!e.target.classList.contains('tab-close')) switchToFile(parseInt(this.dataset.index)); });
            tab.querySelector('.tab-close').addEventListener('click', e => { e.stopPropagation(); closeFile(parseInt(tab.dataset.index)); });
        }
    });
}
...
function updateCurrentFile() {
    ...
    if (!window.editorState.currentFile) {
        ...
        if (fileNameSpan) fileNameSpan.textContent = 'اختر ملفاً للبدء';
        if (langSelect) langSelect.value = 'html';
        return;
    }
    if (input) { input.value = window.editorState.currentFile.content; input.focus(); }
    if (fileNameSpan) fileNameSpan.textContent = window.editorState.currentFile.name;
    if (langSelect) langSelect.value = window.editorState.currentFile.type;
...
function updateProjectDetails() {
    ...
    const dateString = date.toLocaleDateString('ar-EG', { year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' });
    ...
    details.innerHTML = `
        ...
        <div class="project-detail"><span class="detail-label">آخر تعديل:</span><span class="detail-value">${dateString}</span></div>
...
function closeFile(index) {
    if (!window.editorState.files[index]) return;
...
function createNewFile() {
    ...
    const fileLang = langSelect.value;
    const fileContent = contentTextarea.value;
    
    if (!fileName) { showToast('يرجى إدخال اسم الملف', 'error'); nameInput.focus(); return; }
    if (!fileName.includes('.')) { showToast('يرجى إضافة امتداد للملف', 'error'); nameInput.focus(); return; }
    if (window.editorState.files.some(f => f.name === fileName)) { showToast(`يوجد ملف باسم "${fileName}" بالفعل`, 'error'); nameInput.focus(); return; }
    
    let detected = detectLanguage(fileName);
    if (detected === 'unknown') detected = fileLang;
    
    const newFile = {
        name: fileName,
        content: fileContent || getFileTemplate(detected, fileName),
        type: detected,
        active: true,
        unsaved: true
    };
    ...
    window.editorState.currentFile = newFile;
    hideAllModals();
    nameInput.value = ''; contentTextarea.value = '';
    updateEditorUI();
...
function filterFiles(term) {
    ...
    const filtered = window.editorState.files.filter(f => f.name.toLowerCase().includes(lower) || f.type.toLowerCase().includes(lower));
    list.innerHTML = '';
    if (filtered.length > 0) {
        filtered.forEach(file => {
            const idx = window.editorState.files.indexOf(file);
            const item = document.createElement('div');
            ...
            item.innerHTML = `
                ...
                <div class="file-manager-actions">
                    <button class="btn btn-sm btn-primary open-manager-file" data-index="${idx}"><i class="fas fa-edit"></i></button>
                    <button class="btn btn-sm btn-danger delete-manager-file" data-index="${idx}"><i class="fas fa-trash"></i></button>
                </div>
            `;
            list.appendChild(item);
            item.querySelector('.open-manager-file').addEventListener('click', function() { hideAllModals(); switchToFile(parseInt(this.dataset.index)); });
            item.querySelector('.delete-manager-file').addEventListener('click', function() { deleteFile(parseInt(this.dataset.index)); showFileManager(); });
        });
    } else {
        list.innerHTML = `<div class="empty-state" style="padding: 40px 20px;"><i class="fas fa-search" style="font-size:40px; opacity:0.5;"></i><h3>لا توجد نتائج</h3><p>لم يتم العثور على ملفات تطابق "${term}"</p></div>`;
    }
}
...
function saveProject() {
    ...
    try {
        ...
        window.editorState.files.forEach(f => {
            window.editorState.currentProject.files[f.name] = f.content;
            window.editorState.currentProject.fileTypes[f.name] = f.type;
            f.unsaved = false;
        });
...
function startAutoSave() {
    ...
    window.editorState.autoSaveInterval = setInterval(() => { if (window.editorState.isChanged) saveProject(); }, 30000);
}
...
function downloadProject() {
    ...
    else showToast('مكتبة الضغط غير متوفرة', 'error');
}

function createZipFile(project) {
    const zip = new JSZip();
    Object.keys(project.files).forEach(f => zip.file(f, project.files[f]));
    const info = {
...
function formatCode() {
    ...
    let formatted = window.editorState.currentFile.content;
    switch (window.editorState.currentFile.type) {
        case 'html': formatted = formatHTML(formatted); break;
        case 'css': formatted = formatCSS(formatted); break;
        case 'javascript': formatted = formatJS(formatted); break;
        default: showToast('لا يوجد تنسيق خاص لهذه اللغة', 'info'); return;
    }
    window.editorState.currentFile.content = formatted;
    window.editorState.currentFile.unsaved = true;
    window.editorState.isChanged = true;
    updateCurrentFile();
...
function performSearch() {
    ...
    if (matches) {
        ...
        window.editorState.searchResults.slice(0,10).forEach((res,i) => {
            const el = document.createElement('div');
            el.className = 'search-result'; el.dataset.index = i;
            const start = Math.max(0, res.index - 30);
            const end = Math.min(content.length, res.index + res.length + 30);
            const context = content.substring(start, end);
            el.innerHTML = `<div class="search-result-line">${highlightText(context, searchText, matchCase)}</div><div class="search-result-context">السطر: ${getLineNumber(content, res.index)} • الموضع: ${res.index}</div>`;
...
function replaceText() {
    // ... (وظيفة الاستبدال، كما في الكود الأصلي)
}
...
function toggleTheme() {
    ...
    if (body.classList.contains('dark-mode')) {
        ...
        btn.innerHTML = '<i class="fas fa-sun"></i>'; btn.title = 'تبديل الوضع الفاتح';
    } else {
        body.classList.remove('light-mode'); body.classList.add('dark-mode');
        window.editorState.theme = 'dark-mode';
        btn.innerHTML = '<i class="fas fa-moon"></i>'; btn.title = 'تبديل الوضع المظلم';
    }
...
function loadTheme() {
    ...
    if (saved) {
        ...
        btn.title = saved === 'light-mode' ? 'تبديل الوضع المظلم' : 'تبديل الوضع الفاتح';
    }
}
...
function detectLanguage(fileName) {
    ...
    const map = {
        '.html':'html','.htm':'html','.css':'css','.js':'javascript','.jsx':'javascript','.ts':'typescript','.tsx':'typescript',
        '.py':'python','.php':'php','.java':'java','.cpp':'cpp','.c':'c','.cs':'csharp','.rb':'ruby','.swift':'swift','.go':'go','.rs':'rust',
        '.json':'json','.xml':'xml','.sql':'sql','.md':'markdown','.txt':'text'
    };
    return map[ext] || 'unknown';
}
function getFileIcon(type) {
    const icons = {
        'html':'fab fa-html5','css':'fab fa-css3-alt','javascript':'fab fa-js-square','typescript':'fas fa-code',
        'python':'fab fa-python','php':'fab fa-php','java':'fab fa-java','cpp':'fas fa-file-code','c':'fas fa-file-code',
        'csharp':'fas fa-file-code','ruby':'far fa-gem','swift':'fas fa-mobile-alt','go':'fas fa-code','rust':'fas fa-cog',
        'json':'fas fa-code','xml':'fas fa-code','sql':'fas fa-database','markdown':'fas fa-file-alt','text':'fas fa-file-alt',
        'unknown':'fas fa-file'
    };
...
function getLanguageName(code) {
    const names = {
        'html':'HTML','css':'CSS','javascript':'JavaScript','typescript':'TypeScript','python':'Python','php':'PHP','java':'Java',
        'cpp':'C++','c':'C','csharp':'C#','ruby':'Ruby','swift':'Swift','go':'Go','rust':'Rust','json':'JSON','xml':'XML','sql':'SQL',
        'markdown':'Markdown','text':'نص عادي'
    };
    return names[code] || 'غير معروف';
}
function getFileTemplate(lang, name) {
    const t = {
        html: `<!DOCTYPE html>\n<html>\n<head>\n    <meta charset="UTF-8">\n    <title>${name}</title>\n</head>\n<body>\n    <h1>مرحباً بك</h1>\n</body>\n</html>`,
...
function showExitModal() {
    if (window.editorState.isChanged) showModal('exitModal');
    else exitToHome();
}
...
function showToast(message, type = 'info') {
    ...
    setTimeout(() => { if (toast.parentNode) { toast.style.animation = 'toastSlideIn 0.3s ease reverse forwards'; setTimeout(() => toast.remove(), 300); } }, 5000);
}
...
function initMobileControls() {
    ...
    document.addEventListener('click', (e) => {
        if (window.innerWidth <= 768 && sidebar && toggle && bottomMenu) {
            if (!sidebar.contains(e.target) && !toggle.contains(e.target) && !bottomMenu.contains(e.target)) {
    ...
    if (moreBtn && dropdown) {
        ...
        document.addEventListener('click', (e) => { if (!moreBtn.contains(e.target) && !dropdown.contains(e.target)) dropdown.classList.remove('show'); });
    }
...
if ('ontouchstart' in window) {
    ...
    document.querySelectorAll('button').forEach(b => {
        b.addEventListener('touchstart', function() { this.style.transform = 'scale(0.95)'; });
        b.addEventListener('touchend', function() { setTimeout(() => this.style.transform = '', 150); });
    });
}
